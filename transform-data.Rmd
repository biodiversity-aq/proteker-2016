...
#
create_event_emof <- function(data = data, mof_fields = mof_fields, measurement_col) {
  ...
  mof <- data %>% 
    ...
    select(eventID, !!as.symbol(measurement_col))  # occurrenceID is absent because this is an event table
...
```{r message=FALSE}
#' Function to subset a measurementType from wide occurrence table to long measurement or fact table
#'
#' @param data The data frame which contains measurement data in wide format.
#' @param mof_fields The data frame which contains measurement columnName of `data`, measurementType and measurementUnit
#' @param measurement_col A string which is the field name of data frame that contains the measurement value. Will be used to populate the measurementValue field.
...
create_occ_emof <- function(data = data, mof_fields = mof_fields, measurement_col) {
  # create a measurement or fact table of a single measurement type from the data
  # subset from full dataset
  mof <- data %>% 
    # remove rows of measurement which are NULL
...
```{r}
water_temp <- create_event_emof(data = event, mof_fields = mof_fields, measurement_col = "water temperature (°C)")
d13C <- create_occ_emof(data = occ, mof_fields = mof_fields, measurement_col = "d13C")
d15N <- create_occ_emof(data = occ, mof_fields = mof_fields, measurement_col = "d15N")
d34S <- create_occ_emof(data = occ, mof_fields = mof_fields, measurement_col = "d34S")
percent_C <- create_occ_emof(data = occ, mof_fields = mof_fields, measurement_col = "%C")
percent_N <- create_occ_emof(data = occ, mof_fields = mof_fields, measurement_col = "%N")
percent_S <- create_occ_emof(data = occ, mof_fields = mof_fields, measurement_col = "%S")
C_N_ratio <- create_occ_emof(data = occ, mof_fields = mof_fields, measurement_col = "C/N")
